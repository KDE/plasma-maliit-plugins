...
#include "ut_duivirtualkeyboard.h"
...
void Ut_DuiVirtualKeyboard::initTestCase()
{
    // Avoid waiting if im server is not responding
    DuiApplication::setLoadDuiInputContext(false);

    static char *argv[2] = {(char *) "ut_duivirtualkeyboard", (char *) "-software"};
    static int argc = 2;
    app = new DuiApplication(argc, argv);

    QString InputMethodSetting("/Dui/InputMethods/Languages");
    DuiGConfItem item1(InputMethodSetting);

    QStringList langlist;
    langlist << "en_GB" << "fi" << "ar_SA";
    item1.set(QVariant(langlist));

    QString DefaultLanguageSetting("/Dui/InputMethods/Languages/Default");
    DuiGConfItem item2(DefaultLanguageSetting);
    QString defaultlanguage = "en_GB";
    item2.set(QVariant(defaultlanguage));

    QString pixmapDirectory("../../dui-keyboard/theme");
    if (!QFile::exists(pixmapDirectory)) {
        pixmapDirectory = "/usr/share/dui/virtual-keyboard/images";
        QVERIFY(QFile::exists(pixmapDirectory));
    }
    DuiTheme::instance()->addPixmapDirectory(pixmapDirectory);
    ...
    // DuiVirtualkeyboard uses DuiPlainWindow internally so we need to instantiate it.
    new DuiPlainWindow; // creates a static instance
...
void Ut_DuiVirtualKeyboard::cleanupTestCase()
{
...
void Ut_DuiVirtualKeyboard::init()
{
    m_vkb = new DuiVirtualKeyboard(LayoutsManager::instance(), vkbParent);
...
void Ut_DuiVirtualKeyboard::cleanup()
{
...
void Ut_DuiVirtualKeyboard::clickBackspaceTest()
{
    ...
    //TODO:there are bugs in duibutton, we can not simulate the keypress and keyrelease...
...
void Ut_DuiVirtualKeyboard::clickSpaceTest()
{
#if 0
    FlickUpButton *button = 0;
    ...
    QVERIFY(button != 0);
    QVERIFY(m_vkb->shiftLevel == DuiVirtualKeyboard::ShiftOff);
    ...
    m_vkb->clickShift();
    QVERIFY(m_vkb->shiftLevel == DuiVirtualKeyboard::ShiftOn);
    spy.clear();
...
void Ut_DuiVirtualKeyboard::setShiftStateTest()
{
    QList<DuiVirtualKeyboard::ShiftLevel> levels;
    levels << DuiVirtualKeyboard::ShiftOn << DuiVirtualKeyboard::ShiftOff
           << DuiVirtualKeyboard::ShiftLock;
    foreach(DuiVirtualKeyboard::ShiftLevel level, levels) {
        m_vkb->setShiftState(level);
        QCOMPARE(m_vkb->shiftStatus(), level);
    }
}

void Ut_DuiVirtualKeyboard::clickHyphenTest()
{
#if 0
    FlickUpButton *button = 0;
    for (int i = 0; i < numFunctionKeys; i++) {
...
void Ut_DuiVirtualKeyboard::clickPunctQuesTest()
{
#if 0
    FlickUpButton *button = 0;
    for (int i = 0; i < numFunctionKeys; i++) {
...
void Ut_DuiVirtualKeyboard::clickPunctDotTest()
{
#if 0
    FlickUpButton *button = 0;
...
void Ut_DuiVirtualKeyboard::fadeTest()
{
    m_vkb->fade(100);
    QCOMPARE(m_vkb->opacity(), 1.0);
}

void Ut_DuiVirtualKeyboard::regionSuppressionTest()
{
...
void Ut_DuiVirtualKeyboard::showKeyboardTest()
{
    QSignalSpy spy(m_vkb, SIGNAL(regionUpdated(QRegion)));
    m_vkb->showKeyboard();
...
void Ut_DuiVirtualKeyboard::hideKeyboardTest()
{
...
void Ut_DuiVirtualKeyboard::testStateReset()
{
    // Open keyboard.
    m_vkb->showKeyboard();
    QCOMPARE(m_vkb->activity, DuiVirtualKeyboard::Active);

    // Set states that should be changed next time opening vkb.
    m_vkb->setShiftState(DuiVirtualKeyboard::ShiftOn); // Shift on
    ...
    QCOMPARE(m_vkb->shiftStatus(), DuiVirtualKeyboard::ShiftOff); // Shift should be off
}

void Ut_DuiVirtualKeyboard::switchLevelTest()
{
    m_vkb->shiftLevel = DuiVirtualKeyboard::ShiftLock;
    m_vkb->switchLevel();
    QCOMPARE(m_vkb->currentLevel, 1);
}

void Ut_DuiVirtualKeyboard::flickRightHandlerTest()
{
    QStringList langList = DuiGConfItem("/Dui/InputMethods/Languages").value().toStringList();
    langList.sort();
    ...
    //depends on the languages set through DuiGConfItem
...
void Ut_DuiVirtualKeyboard::flickLeftHandlerTest()
{
    QStringList langList = DuiGConfItem("/Dui/InputMethods/Languages").value().toStringList();
    langList.sort();
...
void Ut_DuiVirtualKeyboard::loadSymbolViewTemporarilyTest()
{
...
void Ut_DuiVirtualKeyboard::errorCorrectionTest()
{
#if 0
...
void Ut_DuiVirtualKeyboard::setKeyboardType()
{
    m_vkb->setKeyboardType(Dui::NumberContentType);
    QCOMPARE(m_vkb->currentLayoutType, LayoutData::Number);

    m_vkb->setKeyboardType(Dui::PhoneNumberContentType);
    QCOMPARE(m_vkb->currentLayoutType, LayoutData::PhoneNumber);

    m_vkb->setKeyboardType(Dui::FreeTextContentType);
    QCOMPARE(m_vkb->currentLayoutType, LayoutData::General);

    m_vkb->setKeyboardType(Dui::EmailContentType);
    QCOMPARE(m_vkb->currentLayoutType, LayoutData::General);

    m_vkb->setKeyboardType(Dui::UrlContentType);
    QCOMPARE(m_vkb->currentLayoutType, LayoutData::General);

    m_vkb->setKeyboardType(Dui::CustomContentType);
    QCOMPARE(m_vkb->currentLayoutType, LayoutData::General);
}

void Ut_DuiVirtualKeyboard::longPressBackSpace()
{
#if 0
    FlickUpButton *button = 0;
 ...
 * in uppercase mode
 */
void Ut_DuiVirtualKeyboard::bug_130644()
{
#if 0
    ...
    foreach(const QString & label, buttonLabels) {
        ...
        QCOMPARE(m_vkb->shiftStatus(), DuiVirtualKeyboard::ShiftOff);
        m_vkb->setShiftState(DuiVirtualKeyboard::ShiftOn);
        QVERIFY(m_vkb->shiftStatus() == DuiVirtualKeyboard::ShiftOn);
...
void Ut_DuiVirtualKeyboard::symbolKeyTestLowercase()
{
#if 0
    FlickUpButton *symbolButton = 0;
    QSignalSpy spy(m_vkb, SIGNAL(clickedSymbolButton(const QString &)));
...
void Ut_DuiVirtualKeyboard::symbolKeyTestCapsLock()
{
#if 0
    FlickUpButton *symbolButton = 0;
    ShiftButton *shift = 0;
    ...
    foreach(const QString & label, buttonLabels) {
        //initialization
        qDebug() << "Test button" << label;
        QCOMPARE(m_vkb->shiftStatus(), DuiVirtualKeyboard::ShiftLock);
...
void Ut_DuiVirtualKeyboard::interceptPress()
{
#if 0
 ...
 * is activated
 */
void Ut_DuiVirtualKeyboard::bug_137295()
{
    ...
    m_vkb->lockShift();
    QVERIFY(m_vkb->shiftLevel == DuiVirtualKeyboard::ShiftLock);
    QVERIFY(m_vkb->scv->isActive() == true);
    QVERIFY(m_vkb->scv->isVisible() == true);
...
void Ut_DuiVirtualKeyboard::testSetKeyboardState()
{
    int top = 0;
...
void Ut_DuiVirtualKeyboard::testReactionMaps()
{
    ...
    QVERIFY(tester.testReactionMapGrid(DuiPlainWindow::instance(), 40, 50, m_vkb->region(true)));

    // Check that all buttons are drawn with reactive color.
    QVERIFY(tester.testChildButtonReactiveAreas(DuiPlainWindow::instance(), m_vkb));
...
void Ut_DuiVirtualKeyboard::flickUpHandlerTest_data()
{
...
void Ut_DuiVirtualKeyboard::flickUpHandlerTest()
{
...
void Ut_DuiVirtualKeyboard::rotateToAngle(Dui::OrientationAngle angle)
{
...
QTEST_APPLESS_MAIN(Ut_DuiVirtualKeyboard);
